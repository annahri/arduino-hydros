...
// Nombre de releve
int NB = 10;
char idTotem[] = "T02"; // Changer le nom du Totem ici
const int tailleA = NOMS + 3 + (NBCAPTS * 4); // defaut: 22 octets
const int tailleB = NOMS + 1 + 8 + 2 + 2 + 37; // defaut: 53 octets
...
void setup() {
  ...
  //int envPhoto = timer.every(12*60*60*1000, PrendreEtEnvoyerPhoto);
}
    ...
    if (xbee.readPacket(500)) {
      ...
      if (xbee.getResponse().getApiId() == ZB_TX_STATUS_RESPONSE) {
          ...
          if (txStatus.getDeliveryStatus() == SUCCESS) {
            ...
            // mise en vrai le dataSent, puis incrementer le count  
            dataSent = true;
